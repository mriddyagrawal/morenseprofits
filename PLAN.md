...
- 2026-05-31 — **Phase 8 MCP arc CLOSED at 16/16 tools.** Contract pinned in [DESIGN/PHASE_8_MCP.md](DESIGN/PHASE_8_MCP.md). Sub-arcs 3.4 backtest replay (`backtest_one` c3545cc, `sweep_windows` 96a506c, plus prereq `chore(p8.fill_audit.centralize)` b29d55e + follow-up `docs(p8.mcp.sweep_windows.pre_arc_note)` 8a44bb8), 3.5 diagnostics (`skip_summary` fc6356e + polish f27afb7, `data_quality` 22104df + b25f048 Option-B fix), 3.6 research helpers (`compare_cells` 67c6d3f, `bootstrap_ci` 72d06c6) landed. Plus `chore(p8.cell_stats.centralize)` (ebe7228 — extracted `CellStatsBlock` + `compute_cell_stats` into `src/analytics/cell_stats.py` after the 3rd duplication site preemptively, per reviewer's "fragile at 3" framing). Closed with 4 post-arc polish commits (44eed75 truncation-docs, 0a81f31 schema-cap, f4707e3 entry-side-docs, 69ab7e3 cvar_alpha lock) closing every carry-forward reviewer grill before the docs commit. Final docs commit pins the 16-tool catalog, the cross-cutting honesty policies (caveats contract, no-p-values on compare_cells, multiple-comparisons re-export, pre-pricing-arc detection, ENTRY-side scope on data_quality, lowest-N truncation, cvar_5_roi_pct field-name lock), the operator-side `~/.claude/mcp.json` config, and the forward-compat tripwire (`test_server_registry_now_exposes_bootstrap_ci_and_all_16_tools` LOAD-BEARING pin of the v1 catalog). Test count grew 678 → ~830 over the closing sub-arcs (~152 new tests). Phase 9 (paper trading) is the next surface-area expansion; MUTATING tools land there under a separate registry pin (the v1 read-only contract stays frozen).
