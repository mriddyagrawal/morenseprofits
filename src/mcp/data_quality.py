...
def _theoretical_fallback_rate(df: pd.DataFrame) -> tuple[list[dict], str]:
    ...
    summary = (
        ...
        "BEFORE the turnover-ingest landed in the p7.pricing_arc era "
        "— their fills are the pre-VWAP behaviour (still gate-"
        "corrected, but VWAP refinement absent). Symbols near 0% "
        "close fallback have full VWAP coverage. Mixed coverage was "
        "the expected state after the 2026-05-31 prefetch run; "
        "force-refresh those symbols if uniform VWAP coverage matters "
        "for your comparison. NOTE (post-P1.7, 2026-06-03): under "
        "the current ``p1.7.vwap_or_skip`` engine the close-fallback "
        "branch is gone entirely — sweeps generated after this point "
        "have close_fill_rate_pct = 0 by construction; this caveat "
        "describes only legacy sweeps stamped with earlier engine "
        "versions."
    )
    return rows, summary
