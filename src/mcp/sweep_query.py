...
class RunInfo(BaseModel):
    ...
    engine_version: str | None = Field(
        ...
        description=(
            ...
            "KV metadata. Examples: 'p1.7.vwap_or_skip' (current arc — "
            "VWAP-or-skip fills, no close fallback, Option C liquidity "
            "bypass, F7 oi==0+thin skip — see "
            "``src/engine/results.py``); 'p7.pricing_arc' (prior arc — "
            "IlliquidLegError gate + VWAP fill *with* close fallback); "
            "``None`` for legacy parquets cached before "
            "chore(p8.engine.version_stamp) landed — those are "
            "pre-stamp data with the phantom-fill bias likely present."
        ),
    )
    pricing_arc_applied: bool = Field(
        ...
        description=(
            ...
            "ENGINE_VERSION (currently 'p1.7.vwap_or_skip' — bumped "
            "from 'p7.pricing_arc' in a32c8c0 when P1.7 stripped close "
            "fallback). Use this to identify runs generated by the "
            "current engine semantics versus older runs whose fills "
            "may carry close-fallback bias (p7 era) or phantom-fill "
            "bias (pre-stamp era)."
        ),
    )
