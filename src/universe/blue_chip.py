"""Blue-chip universe — 48 large-cap NSE names with liquid options.

**Why 48 (not 50)**: matches the mid-2024 NIFTY 50 snapshot minus 2
historically-thin-options-liquidity members (BPCL, BRITANNIA — held
out per the original v1 "tighter universe" framing). Originally 40
under the "minus 10 thin names" v1 ask; expanded to 48 in May 2026
once the prefetch pipeline + dashboard could absorb the extra
contracts. Exact stock selection is explicitly a v1 shortcut — the
reporting / analysis quality is what matters; Phase 7 will add a
user-curated-universe skill so the operator can swap this list out
per session (PLAN §3 deferred item).
...
date ~2024-07-01, https://en.wikipedia.org/wiki/NIFTY_50). Selection
is "kinda good" per the v1 ask — not a published-research-grade
composition.
...
# 48 large-cap NSE symbols. Alphabetically sorted for determinism +
...
assert len(_BLUE_CHIP_V1) == 48, (
    f"expected 48 blue-chip symbols, got {len(_BLUE_CHIP_V1)}"
)
assert len(set(_BLUE_CHIP_V1)) == 48, "duplicate symbols in list"
...
def blue_chip(as_of: date) -> list[str]:
    """Sorted list of 48 blue-chip NSE symbols as-of ``as_of``.
