...
**Expiry-day safety check (operator raised it — does an OI gate wrongly kill expiry-day trades? NO):** (a) the gate keys on `entry_oi`, and entry is structurally never expiry day (`entry_offset_td ≥ 1`; empirically the 160 are all `entry_offset_td ∈ {30,33,35,37,41,42}`). (b) Expiry-day OI is **not** ~0 anyway — expiry-day EXIT legs in the priced sweep have median `exit_oi` ≈ **1,144,500** (0.0% are zero). So expiry-day trades are never filtered by an `entry_oi` guard. **And the offset data STRENGTHENS F7:** the 160 are 100% **far-dated entries (T-30 to T-42)** with zero overnight OI — the textbook dead/illiquid far-dated contract (entering a strike 30–42 days out that nobody holds), the *opposite* of a benign near-expiry winddown. That's exactly the fictional fill P1.7's own skip-don't-fudge thesis says to drop.
...
## F9 — spot cache contaminated with NSE "T0" series rows; `load_spot` doesn't filter to EQ

Sanity-checked `data/cache/spot/*` (50 symbols × 3 yrs). Schema/dtypes/units clean, OHLC invariants hold for EQ, and a **strong cross-validation: RELIANCE spot close 2024-08-29 = 3041.85 = the bhavcopy `UndrlygPric` exactly.** BUT some files carry **duplicate dates** (BHEL/2025: 6, PNB/2025: 8) that are **NSE `T0`-series rows** (same-day-settlement segment) sitting beside the `EQ` row: volume 1–3, `open=high=low` single print, `close` copied from EQ (so close-on-dup is coincidentally safe; `open`/`vwap` are degenerate). `spot_loader._normalize` keeps `series` but **`load_spot` never filters to `EQ`** (the `stock_df(series="EQ")` fetch arg doesn't keep T0 out of the cache), so `load_spot` returns 2 rows and the engine takes `iloc[0]` — order-dependent. **Impact:** today only `realized_vol` is affected (dup date → spurious zero log-return → slightly understates vol → optimistic margin; also distorts the `len<20` guard). `entry_spot`/`exit_spot` use `close`, which is coincidentally identical on dups. **Fix:** `df = df[df["series"]=="EQ"]` in `load_spot` (or at cache write). One line; prerequisite for F10.

## F10 — entry uses **close** spot while options fill at **VWAP** (consistency mismatch, not lookahead)

`sweeper.py:194` `spot_at_entry = spot_df.iloc[0]["close"]` drives **ATM strike selection** + **margin notional** (`exit_spot` recorded, not in P&L). Not a lookahead bias (close is known EOD; entry modeled EOD). But post-P1.7 the option leg fills at the day's **premium-VWAP** while the strike+margin reference the day's **close** — two different intraday moments, and the spot cache already has a `vwap` column. Measured close-vs-vwap-spot divergence (EQ-only): median 0.25–0.59%, p90 0.76–1.49%; because strikes are discrete this **flips the selected ATM strike ~24–36% of days** (RELIANCE/SBIN ~30%, PNB ~24%, BHEL ~36%). Not a clear *directional* P&L bias (strategies are ~delta-neutral → CE/PE close-vs-vwap effects largely offset), but real per-trade noise + philosophically inconsistent with the VWAP-everywhere choice. **Operator decision (2026-06-03): switch spot to vwap. Availability CONFIRMED** — across **29,634 EQ rows**: vwap NaN=0, ≤0=0, out-of-band=0, and **0 rows where vwap is missing while close is valid** → always available, no fallback needed (after F9's EQ filter). **Precise scoping — do NOT drop close "entirely":**
- ✅ **Switch to spot `vwap`:** ATM strike selection + margin notional (`sweeper.py:194` `spot_at_entry`) and `exit_spot` (`:198`, recorded-context consistency). Matches the option VWAP fill; shifts ~24–36% of ATM strikes (intended).
- 🚩 **KEEP close-to-close for `realized_vol`** (`vol.py:68-69`). Close-to-close log-return stdev is the *standard* realized-vol definition; vwap-to-vwap is non-standard **and optimistically biased** — measured vwap-vol < close-vol by **1.2–3.9 pp** annualized on all 4 symbols (RELIANCE 50.0→48.8, SBIN 25.9→23.1, PNB 32.9→29.0, BHEL 42.0→39.4): vwap is an intraday average (smoother → lower variance) → margin understated → **ROI overstated**.
- Keep `close` in raw display (`get_spot_series` passthrough).
- **Prereq: F9** (filter `series=="EQ"` in `load_spot`) must land first, else `iloc[0]` can grab a T0 row whose `vwap` is a degenerate single print.
...
- **`ENGINE_VERSION` was NOT bumped for P1.7** (still `"p7.pricing_arc"`, [results.py:41](src/engine/results.py#L41)). A post-P1.7 sweep carries the same stamp as pre-P1.7 — MCP `list_runs`/caveats can't distinguish VWAP-or-skip from close-fallback runs. Minor; worth a bump (e.g. `p1.7.vwap_or_skip`).
- The `new_bhavcopy_format_readme.xlsx` is the official NSE UDIFF spec; `parse_udiff` field names match its "New format" column → fresh re-fetch is safe. Only `options/` MUST be rebuilt for F1 (UDiff `bhavcopy_fo` is already rupees); the cache-version-stamp gap doesn't bite because post-F1 both engine-sources normalize turnover to rupees.

**Open non-blocking:** **F7 (oi==0 drop — surface/filter OR re-skip oi==0-AND-thin; ≤160 trades, optimistic-direction principle violation)** · F3 (expiry STT) · cache-version-stamp guard (Grill #1, dual-concurred) · bhav-vs-raw smoke-gate successor (F6 #1/#2) · 6bc95e9 iterdir-order (minor).
