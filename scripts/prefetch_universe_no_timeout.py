"""Pre-cache NSE options data — NO-POLITENESS-SLEEP variant.

EXPERIMENTAL A/B against scripts/prefetch_universe.py: identical
behavior except the 0.5s NSE politeness sleep in
``src.data.options_loader._fetch_options_json_via_nse_api`` is
monkey-patched to a no-op at module load time. Workers spawned via
mp.Pool re-apply the patch because the monkey-patch sits at the
module top level — every worker re-imports this script under
mp.spawn on macOS, so the patch applies in each child process before
``_process_pair`` is invoked.

USE WITH CAUTION:
  - NSE has a WAF that rate-limits aggressive bursts. Without the
    politeness sleep you may see a wave of 401 / 403 / HTML-challenge
    responses, or get the IP soft-banned for a window.
  - NSE response time itself is the dominant cost (~1-1.5s/call), so
    the upside is bounded: 4 workers × 0.5s sleep removed gives back
    at most ~2s/contract — and ONLY if WAF doesn't trip.
  - This script is for measuring "does the 0.5s sleep meaningfully
    affect total wall-clock?" NOT for production use. If it speeds
    things up cleanly, the result is a calibration data point for a
    future tunable politeness constant — don't just delete the sleep
    upstream from one A/B.
...
Usage:
    python scripts/prefetch_universe_no_timeout.py --workers 4
"""
...
# === PROCESS-WIDE TIME.SLEEP MONKEY-PATCH (load-bearing) ===
#
# options_loader._fetch_options_json_via_nse_api has `time.sleep(0.5)`
# before each NSE call (the politeness delay). We replace
# ``options_loader.time.sleep`` with a no-op, but BE AWARE: every
# Python module that does ``import time`` shares the SAME time module
# object, so this assignment also no-ops ``time.sleep`` for every
# other library in the process (jugaad-data's retry backoff,
# requests' connection-pool retries, tqdm's refresh throttle, etc.).
# Acceptable for a one-off speed-test script — the goal is to measure
# wall-clock with zero throttling anywhere — but DO NOT use this
# pattern in production code where some sleeps are legitimately
# load-bearing.
#
# Why this is module-level (not inside main()):
#   On macOS, mp.spawn re-executes the script from scratch in each
#   worker process. Module-level monkey-patch runs in BOTH the parent
#   AND every worker on first import. If this lived inside main(),
#   workers would still hit the upstream 0.5s sleep.
options_loader.time.sleep = lambda *_args, **_kw: None  # type: ignore[assignment]
